<template>
  <div v-if="rol === 'ADMIN'">
        <h1 class="text-center">Reporte de Bebidas</h1>
        <hr>
        <div class="row">
            <div class="col-sm-3">
...
export default {
    ...
    methods:{
        async listarP(){
            ...
            try {
                const res = await axios.get('/reportebeb/'+ ord);
                bebidas = res.data;
                this.generarPDF(bebidas);
            } catch (error) {
        ...
        generarPDF(bebidas){
            ...
            if (bebidas.length > 0) {
                var data = [];
                bebidas.forEach(row => {
                    var r = [row.nombre, row.contenido , row.precio, row.stock];
                    data.push(r);
                });
                const doc = new jsPDF({
