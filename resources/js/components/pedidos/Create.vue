<template>
    <div v-if="rol === 'ADMIN' || rol === 'CAJERA' || rol === 'MESERO'">
        ...
        <span class="text-danger" v-if="errores.total_pagar">{{errores.total_pagar[0]}}</span>

        <div class="row">
            <dir class="d-grid gap-2">
                <button @click="guardar();" type="button" class="btn btn-success text-white">Guardar</button>
            </dir>
        </div>
        <div class="row">
            <dir class="d-grid gap-2">
    ...
    methods: {
        ...
        async guardar(){
            try {
                ...
                const res = await axios.post('/pedido', data);
