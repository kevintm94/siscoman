<template>
  <div v-if="rol === 'ADMIN'">
        <h1 class="text-center">Reporte de guarniciones</h1>
        <hr>
        <div class="row">
            <div class="col-sm-3">
...
export default {
    ...
    methods:{
        async listarP(){
            ...
            try {
                const res = await axios.get('/reportegua/'+ ord);
                guarniciones = res.data;
                this.generarPDF(guarniciones);
            } catch (error) {
        ...
        generarPDF(guarniciones){
            var today = new Date();
            var date = today.getFullYear()+'-'+(today.getMonth()+1)+'-'+today.getDate();
            if (guarniciones.length > 0) {
                var data = [];
                guarniciones.forEach(row => {
                    var r = [row.nombre,row.precio, row.stock];
                    data.push(r);
                });
                const doc = new jsPDF({
